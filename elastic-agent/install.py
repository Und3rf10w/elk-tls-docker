...
if __name__ == "__main__":
    ...
    preflight = True if os.environ.get('PREFLIGHT_CHECK') else False
    Get 
    os.system(f"wget https://artifacts.elastic.co/downloads/beats/elastic-agent/elastic-agent-{os.environ.get('ELK_VERSION')}-linux-x86_64.tar.gz 1>/proc/1/fd/1 2>/proc/1/fd/2")
    os.system(f"tar zxvf elastic-agent-{os.environ.get('ELK_VERSION')}-linux-x86_64.tar.gz 1>/proc/1/fd/1 2>/proc/1/fd/2")
    ...
    os.system(f"/elastic-agent-{os.environ.get('ELK_VERSION')}-linux-x86_64/./elastic-agent install -f --fleet-server-es={os.environ.get('ELASTICSEARCH_HOSTS')} --fleet-server-service-token={os.environ.get('FLEET_SERVER_SERVICE_TOKEN')} --fleet-server-policy={os.environ.get('FLEET_SERVER_POLICY')} --fleet-server-es-ca=/ca.crt --fleet-server-cert=/fleet-server.crt --fleet-server-cert-key=/fleet-server.key --url={os.environ.get('FLEET_URL')} --certificate-authorities=/ca.crt --insecure 1>/proc/1/fd/1 2>/proc/1/fd/2")
    ...
    # Wait 30 seconds just in case the enroll fails to execute the elastic-agent
    time.sleep(30)
    # Finally, run elstic agent
    os.system(f"/usr/bin/elastic-agent run 1>/proc/1/fd/1 2>/proc/1/fd/2 &")
    while True:
